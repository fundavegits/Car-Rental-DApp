export default function RentalHistory({ expanded, data, open, close }) {
  // EXPANDED VIEW: Full transaction ledger
  if (expanded) {
    return (
      <div style={{ animation: "fadeIn 0.4s ease" }}>
        ...
        <div style={{ background: "#121216", borderRadius: "20px", border: "1px solid #222", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ textAlign: "left", background: "#1c1c21", color: "#888" }}>
                <th style={{ padding: "20px" }}>VEHICLE</th>
                <th style={{ padding: "20px" }}>COMPLETED DATE</th>
                <th style={{ padding: "20px", textAlign: "right" }}>AMOUNT PAID</th>
              </tr>
            </thead>
            <tbody>
              {data.map((item, i) => (
                <tr key={i} style={{ borderBottom: "1px solid #222" }}>
                  ...
                  <td style={{ padding: "20px", textAlign: "right", color: "#a855f7", fontWeight: "bold" }}>
                    {item.paid} ETH
                  </td>
                </tr>
              ))}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        {data.slice(0, 5).map((item, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #222" }}>
            <p style={{ margin: 0, fontSize: "0.85rem", color: "#ccc" }}>{item.model}</p>
            <span style={{ color: "#a855f7", fontWeight: "bold", fontSize: "0.85rem" }}>Ξ {item.paid}</span>
          </div>
        ))}
      </div>
    </div>
