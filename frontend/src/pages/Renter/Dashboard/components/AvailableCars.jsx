...
export default function AvailableCars({ filters, onAutoFill }) {
  ...
  // SCROLL LOCK: Prevents background mess when modal is open
  useEffect(() => {
    ...
    <div className="card" style={{ background: "rgba(255, 255, 255, 0.05)", padding: "20px", borderRadius: "15px", border: "1px solid #222" }}>
      ...
      {isModalOpen && (
        <div style={{
          ...
          width: "100vw", height: "100vh", backgroundColor: "rgba(0,0,0,0.95)",
          display: "flex", justifyContent: "center", alignItems: "center",
          zIndex: 99999, backdropFilter: "blur(15px)", padding: "20px"
        }}>
          <div style={{
            background: "#0a0a0c", padding: "40px", borderRadius: "24px",
            width: "100%", maxWidth: "1200px", maxHeight: "85vh", overflowY: "auto",
            border: "1px solid #333", boxShadow: "0 0 50px rgba(0,0,0,1)"
          }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '25px' }}>
              {filteredCars.map(car => (
                <CarCard key={car.id} car={car} bookingDates={filters} onAutoFill={(data) => { onAutoFill(data); setIsModalOpen(false); }} />
              ))}
            </div>
          </div>
        </div>
